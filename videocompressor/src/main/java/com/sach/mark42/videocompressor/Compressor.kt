...
class Compressor(private val context: AppCompatActivity) {

    private val FRAME_RATE = 30
    private val I_FRAME_INTERVAL = 15
    private val MIME_TYPE = "video/avc"

    var isRunning = true

    fun compress(srcPath: String, destPath: String, listener: CompressionListener) {
        isRunning = true
        startCompressInIOThread(srcPath, destPath, listener).observe(context, Observer {
            if (it) {
                listener.onSuccess()
            } else {
                listener.onFailure()
    ...
    private fun startCompressInIOThread(srcPath: String, destPath: String, listener: CompressionListener) : LiveData<Boolean>{
        val resultLiveData = MutableLiveData<Boolean>()
        GlobalScope.launch(Dispatchers.IO) {
            val result = compressVideo(srcPath, destPath, object : CompressionProgressListener {
    ...
    private fun compressVideo(
        ...
        Log.v("TestVideo", "rotation: $rotation")
        Log.v("TestVideo", "bitrate: $bitrate")
